...
Determines the Execution Modes supported by the application.
  - adhoc: the application will poll the ECS API once and then print and report (if configured) its findings
  - periodic: the application will poll the ECS API on an interval (polling-interval-seconds) and report (if configured) its findings
*/
package mode
