...
func Post(requestBody []byte, id string, path string, anchoreDetails connection.AnchoreInfo, operation string) (*[]byte, error) {
	defer tracker.TrackFunctionTime(time.Now(), fmt.Sprintf("Sent %s request to Anchore", operation))

	logger.Log.Debugf("Performing %s to Anchore using endpoint: %s", operation, strings.Replace(path, "{{id}}", id, 1))
...
func getURL(anchoreDetails connection.AnchoreInfo, path string, id string) (string, error) {
...
func checkHTTPErrors(response *http.Response, operation string) error {
	...
	case response.StatusCode >= 400 && response.StatusCode <= 599:
		...
		// Depending on where an error is discovered during request processing on the server, the
		// error information in the response will be either an APIErrorDetails or a ControllerErrorDetails
...
func getBody(response *http.Response, operation string) (*[]byte, error) {
	...
	if err != nil {
		...
		logger.Log.Debugf("%s %v", operation, errMsg)
	...
	if len(responseBody) > 0 && !json.Valid(responseBody) {
		logger.Log.Debugf("Anchore %s response body: %s", operation, string(responseBody))
